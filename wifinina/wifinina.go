...
const (
	...
	*/
	...
	ErrTimeoutDeviceReady  Error = 0x01
	ErrTimeoutDeviceSelect Error = 0x02
...
func (d *Device) StartClient(addr uint32, port uint16, sock uint8, mode uint8) error {
	...
	if err := d.waitForDeviceSelect(); err != nil {
		d.spiDeviceDeselect()
		return err
	}
...
func (d *Device) waitForDeviceSelect() (err error) {
	err = d.waitForDeviceReady()
	if err == nil {
		err = d.spiDeviceSelect()
	}
	return
}
...
func (d *Device) waitForDeviceReady() error {
	if _debug {
		println("waitForDeviceReady()\r")
	}
	for t := newTimer(10 * time.Second); !(d.ACK.Get() == false); {
		if t.Expired() {
			return ErrTimeoutDeviceReady
		}
	}
	return nil
}

func (d *Device) spiDeviceSelect() error {
	if _debug {
		println("spiDeviceSelect()\r")
	}
	d.CS.Low()
	...
	return ErrTimeoutDeviceSelect
}

func (d *Device) spiDeviceDeselect() {
	if _debug {
		println("spiDeviceDeselect\r")
	}
	d.CS.High()
}
