...
// Only the first 4 letters would be shown.
...
// on the display with a specific position (0-3).
...
// Only the last 4 digit of the number would be shown.
// For negative numbers, only -999 and above would be
...
func pinMode(pin machine.Pin, mode bool) {
	if mode {
		...
		// (because we did so in the beginning.)
